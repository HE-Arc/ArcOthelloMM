...
{
    /// <summary>
    /// This class is used to simulate plays for the alphabeta algorithm
    /// </summary>
    class TreeNode
    {
        public int[,] Board;
        ...
        {
            CurrentValue = CurrentValue == Tools.WHITE ? Tools.BLACK : Tools.WHITE;
            UpdateListPossibleMoves();
        }
        ...
        public void Show()
        {
            ...
            for (int y = 0; y < Board.GetLength(1); ++y)
            {
                for (int x = 0; x < Board.GetLength(0); ++x)
                {
                    ...
                    {
                        if (value == Tools.WHITE || value == Tools.BLACK)
                            Console.Write(value);
                        else
                            Console.Write("-");
                    }
        ...
        {
            ListPossibleMoves = Tools.GetPossibleMoves(Board, CurrentValue == Tools.WHITE);
        }
        ...
        public int Evaluate(int playerValue)
        {
            ...
            // Add points if the current player will play the last token
            heuristic += 50 * ((OpponentToken.Count + CurrentToken.Count + 1) % 2); 
